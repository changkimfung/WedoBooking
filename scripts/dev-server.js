...
var inventoryService = require('./lib/inventoryInstructionService');
...
var server = http.createServer(function (req, res) {
  ...
  if (req.method === 'POST' && req.url === INVENTORY_INSTRUCTION_ACTION_PATH) {
    readBody(req)
      .then(function (body) {
        ...
        var result = inventoryService.applyInventoryAction(list, body);
        if (result.newRecord) {
        ...
        sendJson(res, 200, { ok: true, list: list, instruction: result.instruction });
      })
  ...
  if (req.method === 'GET' && req.url === INVENTORY_INSTRUCTION_API_PATH) {
    try {
      ...
      var changed = inventoryService.normalizeInstructionList(instructionList);
  ...
  if (req.method === 'POST' && req.url === INVENTORY_INSTRUCTION_API_PATH) {
    readBody(req)
      .then(function (body) {
        ...
        inventoryService.expandWarehouseTasks(body.list, stock);
