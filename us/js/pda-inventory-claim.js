(function () {
  ...
  function skuRows() {
    ...
    return Object.keys(groups).map(function (sku) {
      ...
      return { skuCode: sku, pendingCount: items.length, owner: active.length ? active[0].claimedBy : '', active: active.length > 0 };
    }).sort(function (a, b) {
  ...
  function render() {
    ...
    document.getElementById('skuBody').innerHTML = rows.map(function (row) {
      ...
      return '<tr><td>' + esc(row.skuCode) + '</td><td>' + row.pendingCount + '</td><td>' + status + '</td><td>' + operation + '</td></tr>';
